...
type LogOperation struct {
	...
	Country   string `gorm:"type:varchar(255)"`
	City      string `gorm:"type:varchar(255)"`
	ZhName    string `gorm:"type:varchar(30)"`
	EnName    string `gorm:"type:varchar(80)"`
}
...
func (l *LogModel) CreateOperationLog(userId uint, operation string, ip string, country string, city string, zhName, enName string) (uint, error) {
	data := LogOperation{
		UserId:    userId,
		Operation: operation,
		Ip:        ip,
...
func (this *LogModel) GetLastLoginLog(userId int64) (*LogLogin, error) {
	var info LogLogin

	err := DB.Model(&LogLogin{}).Where("user_id = ?", userId).
